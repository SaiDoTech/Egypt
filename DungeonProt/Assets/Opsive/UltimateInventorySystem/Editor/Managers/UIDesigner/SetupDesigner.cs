namespace Opsive.UltimateInventorySystem.Editor.Managers.UIDesigner
{
    ...
    {
        public override string DocumentationURL => "https://opsive.com/support/documentation/ultimate-inventory-system/editor-window/ui-designer-schemas/";
        ...
        public UIDesignerSchemaSetupSubMenu()
        {
            ...
            if (defaultValue == null) {
                Debug.LogError("ERROR: No Schemas were found in the project, make sure at least one exist. Try Reimporting the UIDesignerSchema asset if one does exist");
                return;
            }
        ...
        public void Refresh()
        {
            ...
            if (schema == null) {
                ...
                m_HelpBox.SetMessage("A UI Designer schema must be created and assigned to use the UI Designer Editor Manager.");
            ...
            if (editorResult.isValid == false) {
                m_HelpBox.SetMessage("The selected schema is not valid, the error is:\n"+editorResult.message);
                m_ValidateResultContainer.Add(m_HelpBox);
                return;
            }
        ...
        private void FullUISetup()
        {
            ...
            var schemaInstance = UIDesignerManager.InstantiateSchemaPrefab<RectTransform>(UIDesignerSchema.FullSchemaLayout, rectParent);
            ...
            for (int i = 0; i < schemaInstance.transform.childCount; i++) {
                ...
                if (child.name == "Gameplay Panel") {
                    panelManager.GameplayPanel = child.GetComponent<DisplayPanel>();
                }

                if (child.name == "Main Menu") {
                    panelManager.MainMenu = child.GetComponent<DisplayPanel>();
                }
            }
        }
        ...
        private void ValidateSchema()
        {
            ...
            if (UIDesignerManager.UIDesignerValidationResult.isValid) { Debug.Log("The UI Designer Schema is valid!"); } else {
        ...
        private void UpdateUIDesignerSchema(UIDesignerSchema schema)
        {
            ...
            if (m_SchemaPopupField.value != UIDesignerSchema) {
                ...
                if (m_Schemas.Contains(UIDesignerSchema) == false) {
                    m_Schemas.Add(UIDesignerSchema);
                }
        ...
        private void DuplicateSchema(UIDesignerSchema originalSchema)
        {
            if (originalSchema == null) {
                Debug.LogWarning("You must select a UI Designer Schame to duplicate in the object field.");
                return;
            }

            var path = EditorUtility.SaveFilePanel("Duplicate UI Designer Schema", "Assets", "UIDesignerSchema", "asset");
    ...
    public class UIDesignerSchemaDuplicator
    {
        ...
        private UIDesignerSchema Duplicate(UIDesignerSchema sourceSchema, string newSchemaPath)
        {
            ...
            m_NewUIDesignerSchema = (UIDesignerSchema)AssetDatabase.LoadAssetAtPath(newSchemaPath, typeof(UIDesignerSchema));
            ...
            foreach (var keyValuePair in schemaSourceToNewObjectsDictionary) {
                ...
                if (duplicateGameObject == null) {
                    if (duplicate is Component component) {
                        duplicateGameObject = component.gameObject;
                    } else {
                ...
                for (int i = 0; i < components.Length; i++) {
                    ...
                    foreach (var fieldInfo in componentFields) {
                        ...
                        if (serializedObject == null || schemaSourceToNewObjectsDictionary.TryGetValue(serializedObject, out var duplicateMatch) == false) {
                            continue;
                        }
        ...
        {
            EditorUtility.DisplayProgressBar("Duplicating the UI Designer Schema", "This process may take a while.", percentage);
        }
    }
}